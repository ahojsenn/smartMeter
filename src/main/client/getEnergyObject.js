...
function getEnergyObject() {
  const EnergyObject = this
  const nrOfLines = 1000
  const gotInitialDataEvent = 'gotInitialDataEvent'
  const gotNewDataEvent = 'gotNewDataEvent'
  ...
  this.getData = function (filter) {
    let url = global.url + '/getData?' + 'nolines=' + nrOfLines
    if (filter) url += '&filter=' + filter
    else url += '&filter='

    console.log(
      'in getData, url=',
      global.url + '/getData?' + 'nolines=' + nrOfLines + '&filter=' + filter
    )
    $.ajax({
      url,
      cache: false,
  ...
  this.listenOnWebSocket = function (webSocketDomain, filter) {
    ...
    const socket = io.connect(webSocketDomain)
    socket.on('tailDB', function (d) {
      console.log('\nWebSocket speaks:', JSON.stringify(d))
      // only act, if filter critera is met
      if (typeof filter === 'string' && !JSON.stringify(d).includes(filter))
        //		  				d.indexOf(filter) == -1 )
        return
